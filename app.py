...
# 🟢 [版本號] v6.9 (Deploy-Fix: Port Binding)
BOT_VERSION = "v6.9 (Stable-Deploy)"
...
# --- 安全讀取 Token (避免 NoneType Error) ---
token = os.environ.get('LINE_CHANNEL_ACCESS_TOKEN')
secret = os.environ.get('LINE_CHANNEL_SECRET')

if not token or not secret:
    print("❌ 錯誤: LINE Token 未設定！程式可能無法啟動。")
...
# --- AI 核心 ---
def call_gemini_v6_9(prompt, is_search=False):
...
def get_stock_id(u_input):
    ...
    res, status = call_gemini_v6_9(prompt, is_search=True) 
...
def handle_message(event):
    ...
    if u_text.lower() == "debug":
        token = os.environ.get('FINMIND_TOKEN', '')
        ai_res, ai_status = call_gemini_v6_9("Hi", is_search=True)
    ...
    prompt = (
        ...
        f"【指令】：\n"
        f"1. **格式**：請用「1. 2. 3.」列點，**不要**使用 Markdown 標題。\n"
        f"2. **完整性**：請確保最後一句話有講完。\n"
        f"3. **內容**：\n"
        f"   1. 趨勢與量價 (多空判斷)\n"
        f"   2. 估值與籌碼 (EPS與法人動作)\n"
    ...
    ai_ans, status = call_gemini_v6_9(prompt)
    
    reply = (
        f"📊 **{display_name} **\n"
        f"💰 現價: {tech['close']} | 量比: {tech['vol_ratio']}x\n"
...
if __name__ == "__main__":
    # 抓取 Zeabur 提供的 Port，如果沒有就用 8080
    port = int(os.environ.get("PORT", 8080))
    # 0.0.0.0 代表允許外部連線 (Zeabur 需要這個)
